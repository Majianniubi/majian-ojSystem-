...
public class QuestionController {
    ...
    public BaseResponse<Long> addQuestion(@RequestBody QuestionAddRequest questionAddRequest, HttpServletRequest request) {
        ...
        List<JudgeCase> caseList=questionAddRequest.getJudgeCase();
        if(caseList!= null){
            final String caseStr = JSONUtil.toJsonStr(caseList);
            question.setJudgeCase(caseStr);
        }
        final JudgeConfig judgeConfig = questionAddRequest.getJudgeConfig();
        if(judgeConfig !=null){
            final String configStr = JSONUtil.toJsonStr(judgeConfig);
            question.setJudgeConfig(configStr);
        }
    ...
    public BaseResponse<Boolean> updateQuestion(@RequestBody QuestionUpdateRequest questionUpdateRequest) {
        ...
        Question oldQuestion = questionService.getById(id);
        ThrowUtils.throwIf(oldQuestion == null, ErrorCode.NOT_FOUND_ERROR);
        List<JudgeCase> caseList=questionUpdateRequest.getJudgeCase();
        if(caseList!= null){
            final String caseStr = JSONUtil.toJsonStr(caseList);
            question.setJudgeCase(caseStr);
        }
    ...
    public BaseResponse<Boolean> editQuestion(@RequestBody QuestionEditRequest questionEditRequest, HttpServletRequest request) {
        ...
        Question oldQuestion = questionService.getById(id);
        ThrowUtils.throwIf(oldQuestion == null, ErrorCode.NOT_FOUND_ERROR);
        List<JudgeCase> caseList=questionEditRequest.getJudgeCase();
        if(caseList!= null){
            final String caseStr = JSONUtil.toJsonStr(caseList);
            question.setJudgeCase(caseStr);
        }
    ...
    @GetMapping("/get/submitQuestion/vo")
    public BaseResponse<QuestionSubmitVo> getSubmitQuestionVOById(long id, HttpServletRequest request) {
        if (id <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        final User loginUser = userFeignClient.getLoginUser(request);
        QuestionSubmit submitQuestion = questionSubmitService.getById(id);
        if (submitQuestion == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR);
        }
        return ResultUtils.success(questionSubmitService.getQuestionVO(submitQuestion, loginUser));
    }
}
